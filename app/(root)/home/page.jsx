      ...
      <section className="flex items-center justify-center w-full py-8 sm:py-24">
        <div className="flex flex-col items-center justify-center px-7 py-1 lg:px-20">
          <p className="text-[16px] text-center font-allenoire text-[#07A750] sm:text-[30px] lg:text-[55px]">
            <AnimatedCounter endValue={10} duration={1000} /> Lakhs+
          </p>
...
//     if (data?.data?.blocks && data.data.blocks.length >= 6) {
//       setHeroSliderData(data.data.blocks[0]?.heroslider ?? []);
//       setServiceData(data.data.blocks[0]?.slider ?? []);
//       setProjectData(data.data.blocks[1]?.tabs ?? []);
//       setTestimonialData(data.data.blocks[2]?.testimonialcard ?? []);
//       setNewsData(data.data.blocks[3]?.newstab ?? []);
//       setVideoData(data.data.blocks[4]?.videos ?? []);
//     }
//   }, [data]);
